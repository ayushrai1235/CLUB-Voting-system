...
// Helper function to get candidates with dynamic vote counts
const getCandidatesWithVotes = async (positionId, electionId) => {
  ...
  return candidatesWithVotes.sort((a, b) => b.voteCount - a.voteCount);
};
  ...
  try {
    ...
    const results = await Promise.all(
      positions.map(async (position) => {
        const candidates = await getCandidatesWithVotes(position._id, election._id);
